...
export function InternetIdentityProvider({
  ...
  // Store everything in refs so the init effect only ever runs once
  ...
  // Runs exactly once — no re-initialization loop
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const client = await buildAuthClient(createOptionsRef.current);
        if (cancelled) return;
  ...
  const login = useCallback(() => {
    const client = authClientRef.current;
    if (!client) {
      setErrorMessage("AuthClient is not ready yet. Please try again.");
      return;
    }
